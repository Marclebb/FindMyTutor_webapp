 ...
 return ( 
  ...
  <div className='font-sans grid lg:grid-cols-2 lg:gap-2 md:grid-cols-2 md:gap-2' >
    {cards && cards.length>0 && cards.map (card => (
      <div  className='m-3 border-4 rounded-lg text-slate-800 hover:shadow-md bg-white' >
  ...
  <p className="font-bold text-lg">{card.firstname} {card.lastname}</p>
        ...
        <p className="pl-3 pr-3 m-3">{card.Date}</p>
        <p className="pl-3 pr-3 m-3">{card.time_range_value}</p>
        <p className="pl-3 pr-3 m-3">{card.learning_way}</p>
        <p className="pl-3 pr-3 m-3">{card.learning_method}</p>
...
<path d="M16 8L12 4M12 4L8 8M12 4L12 16" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    {role ==='tutor' && (
      ...
      You canot see what other tutors posted
    </div>
    )}
